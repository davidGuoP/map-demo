...
//        for ($i = 1; $i < 8; $i++) {
//            // 时间段逻辑判断
//            $number = $this->getHourNumber($int_time, $i);
//            $gtCount = $this->getHourGTCount($int_time, $i);
//            $dataType = 0;
//            $countNum = 0;
//            // 判断某人时间人数加起来人数
//            $infoDay = self::$dataReserveInfoRepository->getCount(['int_time' => Common::getIntTime($int_time), 'type' => 1]);
//            $infoHour = self::$dataReserveInfoRepository->getCount(['int_time' => Common::getIntTime($int_time), 'hour' => $i, 'type' => 2]);
//            if ($infoDay || $infoHour) {
//                $dataType = 2;
//            } else if ($number >= config('config.reserve_number') || ($gtCount == 0 && $number >= 16)) {
//                $dataType = 1;
...
//                $countNum = config('config.reserve_number') - $number;
//            }
//            $temp[] = ['play' => $i, 'data_type' => $dataType, 'count' => $countNum];
//        }
        return $temp;
    }
